...
void logControlProcess()
{
  ...
  switch(p.crtp_data[0])
  {
    case CONTROL_CREATE_BLOCK:
      ret = logCreateBlock( p.crtp_data[1],
                            (struct ops_setting*)&p.crtp_data[2], 
                            (p.size-2)/sizeof(struct ops_setting) );
      break;
    case CONTROL_APPEND_BLOCK:
      ret = logAppendBlock( p.crtp_data[1],
                            (struct ops_setting*)&p.crtp_data[2], 
                            (p.size-2)/sizeof(struct ops_setting) );
      break;
    case CONTROL_DELETE_BLOCK:
      ret = logDeleteBlock( p.crtp_data[1] );
      break;
    case CONTROL_START_BLOCK:
      ret = logStartBlock( p.crtp_data[1], p.crtp_data[2]*10);
      break;
    case CONTROL_STOP_BLOCK:
      ret = logStopBlock( p.crtp_data[1] );
      break;
    case CONTROL_RESET:
      logReset();
      ret = 0;
